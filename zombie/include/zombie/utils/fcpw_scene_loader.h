...
void populateGeometricQueries(GeometricQueries<DIM>& geometricQueries,
	...
	geometricQueries.computeDistToDirichlet = [dirichletAggregate, &boundingBox](const Vector<DIM>& x, bool computeSignedDistance) -> float {
	...
	geometricQueries.computeDirichletDisplacement = [](const Vector<DIM>& x) -> zombie::Differential<Vector<DIM>> {
		return zombie::Differential<Vector<DIM>>(Vector<DIM>::Zero());
	};
	geometricQueries.computeNeumannDisplacement = [](const Vector<DIM>& x) -> zombie::Differential<Vector<DIM>> {
		return zombie::Differential<Vector<DIM>>(Vector<DIM>::Zero());
	};
}
