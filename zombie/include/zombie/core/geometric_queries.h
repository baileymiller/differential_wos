...
struct GeometricQueries {
	...
	std::function<Differential<Vector<DIM>>(const Vector<DIM>&)> computeDirichletDisplacement;
	std::function<Differential<Vector<DIM>>(const Vector<DIM>&)> computeNeumannDisplacement;
};
